...
def analytics_view(request):
    ...
    if attendance_percentage > 80:
        ai_insights.append(
            "Overall attendance performance is excellent."
        )

    if absent_students > present_students:
        ai_insights.append(
            "Absent students are higher than present students."
        )

    if total_students > 50:
        ai_insights.append(
            "Student strength increased significantly."
        )

    if len(low_attendance_students) > 3:
        ai_insights.append(
            "Many students require attendance attention."
        )

    if len(ai_insights) == 0:
        ai_insights.append(
            "System analytics running normally."
        )
    ...
    context = {
        ...
        "course_values": list(course_counts.values()),
