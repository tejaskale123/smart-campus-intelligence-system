...
def add_attendance(request):
    ...
    if request.method == "POST":
        ...
        if not student_id or not attendance_date or status not in VALID_ATTENDANCE_STATUS:
            messages.error(
                request,
                "Please select a valid student, date, and attendance status."
            )
            return render(
                request,
...
@login_required
@role_guard("admin", "teacher", "student")
